...
- **Confirmare manuală** (migrare `20260707160000_manual_confirm.sql`): coloană `manual_confirm` pe `recurring_transactions` + `installment_plans`. Reutilizează exact mecanismul „De completat" — generarea automată sare peste ele, iar `list_due_variable_*` întorc acum și sloturile `manual_confirm` (filtru `is_variable OR manual_confirm`, recurențele fără restricția `type='expense'` → merge și pe venituri „de încasat manual"). Checkbox „O confirm manual" în formulare (ascuns când e deja variabilă). `DueVariableItem.reason` = variable|manual pt. text. Pentru întârzieri: marchezi tu plătit/încasat.
- **Redesign carduri** (fix nume ascuns de sumă pe telefon): numele mutat pe rând propriu, lățime completă, sus; suma și acțiunile pe rândul de jos. Header întreg clicabil (icon + nume) pt. desfășurare, nu doar numele. Aplicat pe `RecurringList`, `InstallmentList`, `DebtsManager`. Rând nou „Mod" (automat/variabilă/manual) în detalii.
- **Manual `/help`** actualizat: secțiuni noi Datorii + „De completat", extinderi la recurențe/rate variabile + confirmare manuală.
